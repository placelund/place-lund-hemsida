...
export type CardNavItem = {
  ...
  textColor: string;
  links: CardNavLink[];
  href?: string; // Optional href for clickable headers
};
...
const CardNav: React.FC<CardNavProps> = ({
  ...
  const createTimeline = () => {
    ...
    // Filter out undefined/null refs before animating
            ...
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-0.5">
                {item.links?.map((lnk, i) => (
                  ...
                  >
                    <ArrowUpRight className="nav-card-link-icon shrink-0 w-[14px] h-[14px]" aria-hidden="true" />
                    {lnk.label}
                  </a>
                ))}
              </div>
