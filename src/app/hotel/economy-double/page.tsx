...
export default function EconomyDoublePage() {
  ...
  return (
    <main className="min-h-screen">
      <BreadcrumbSchema items={[{ name: 'Hotel', href: '/hotel' }, { name: 'Economy Double', href: '/hotel/economy-double' }]} />
      <section className="relative h-[60vh] min-h-[500px] flex items-center justify-center pt-20">
        <Image src={heroImage} alt="Economy double room at Place Lund Hotel" fill quality={90} sizes="100vw" className="object-cover" priority />
        <div className="absolute inset-0 bg-black/50 z-10" />
        <div className="relative z-20 text-center px-4 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4">Economy Double Room</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              ...
              <div className="relative h-[400px] overflow-hidden rounded-lg">
                <Image src={galleryImages[0] || heroImage} alt="Economy double room" fill quality={85} sizes="(max-width: 768px) 100vw, 50vw" loading="lazy" className="object-cover" />
              </div>
              {/* Gallery Grid - Show ALL remaining images */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {galleryImages.slice(1).map((image, index) => (
                  <div key={index} className="relative h-48 overflow-hidden rounded-lg">
                    <Image src={image} alt={`Economy room view ${index + 2}`} fill quality={85} sizes="(max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw" loading="lazy" className="object-cover" />
                  </div>
                ))}
              </div>
            </div>
